...
def get_plot_limits(tree_df, obstacles_df=None, padding=0.1):
    """Determine appropriate plot limits based on the data in the CSV files."""
    x_min = tree_df['x'].min()
    x_max = tree_df['x'].max()
    y_min = tree_df['y'].min()
    y_max = tree_df['y'].max()
    
    # Consider obstacles in limits calculation if available
    if obstacles_df is not None:
        for _, obstacle in obstacles_df.iterrows():
            x_min = min(x_min, obstacle['x'])
            x_max = max(x_max, obstacle['x'] + obstacle['width'])
            y_min = min(y_min, obstacle['y'])
            y_max = max(y_max, obstacle['y'] + obstacle['height'])
...
def visualize_tree(tree_df, obstacles_df=None, title="RRT OpenMP Visualization", animate=True, save_animation=False):
    if tree_df is None:
        return
    ...
    x_min, x_max, y_min, y_max = get_plot_limits(tree_df, obstacles_df)
    ...
    if animate:
        ...
        for t in time_steps:
            ...
            if save_animation:
                plt.savefig(f"rrt_omp_frame_{len(lines):04d}.png")
    else:
        # Draw the final tree
...
def main():
    ...
    tree_file = "rrt_obstacles_omp_tree.csv"
    obstacles_file = "rrt_obstacles_omp.csv"
    ...
    if tree_df is not None:
        ...
        visualize_tree(tree_df, obstacles_df, title="RRT with Obstacles (OpenMP)", animate=True, save_animation=False)
